...
struct QuotesListView: View {
    ...
    var body: some View {
            ...
            HStack {
                ...
                Button(isEditing ? "Update" : "Create") {
                    if isEditing {
                        ...
                        opportunity.quotes?.append(quote)
                        text = ""
                        page = ""
                    }
                }
        ...
        .padding(.horizontal)
        List {
            let sortedQuotes = opportunity.quotes?.sorted(using: KeyPathComparator(\Quote.creationDate)) ?? []
            ...
            .onDelete { indexSet in
                withAnimation {
                    indexSet.forEach { index in
                        ...
                        opportunity.quotes?.forEach({ bookQuote in
...
#Preview {
    ...
    let books = Opportunity.sampleOpportunities
    preview.addExamples(books)
    return NavigationStack {
        QuotesListView(opportunity: books[4])
